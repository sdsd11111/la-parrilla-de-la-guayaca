import * as React from 'react';

export const HeroBebidas = () => {
    return (
        <section className="relative h-screen w-full overflow-hidden bg-[#1A1A1A]">
            {/* Background with gradient overlay */}
            <div
                className="absolute inset-0 z-0"
                style={{
                    background: 'linear-gradient(135deg, #1A1A1A 0%, #0D1B2A 100%)', // Slight cool tone for drinks
                }}
            >
                {/* Decorative elements */}
                <div className="absolute top-0 left-0 w-full h-full opacity-10">
                    <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-[#4FC3F7] blur-[128px]" /> {/* Light blue hint */}
                    <div className="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-[#F49D00] blur-[128px]" />
                </div>
            </div>

            {/* Content */}
            <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
                <h1
                    className="text-5xl md:text-7xl font-bold mb-6 tracking-tight"
                    style={{ color: '#FFFFFF' }}
                >
                    Bebidas Refrescantes
                </h1>
                <p
                    className="text-xl md:text-2xl max-w-2xl mx-auto font-light"
                    style={{ color: 'rgba(255, 255, 255, 0.9)' }}
                >
                    El acompañante perfecto para tu parrillada.
                </p>

                {/* Decorative line */}
                <div className="mt-8 w-24 h-1 rounded-full bg-[#F49D00]" />
            </div>
        </section>
    );
};
